...
그런데 `simpl.`을 적용해도 딱히 뭐가 변하지 않는다. 이전의 `0+n`은 plus의 정의에서 바로 치환이 가능했지만 이번엔 딱히 변화가 없다. 다시 종이와 펜을 들고있다고 상상해보자. \\(P \\rightarrow Q\\) 를 증명할 때 \\(P \\rightarrow Q\\)를 바로 보이는 것과, 일단 \\(P\\)를 참이라고 가정하고 \\(Q\\)가 참임을 보이는 것 중 어느 쪽이 편리할까? 후자를 위해 `n = m`을 전제조건으로 포함시키자.
...
전제조건 H에서 n=m이라고 했으므로 n을 m으로 바꿔써도 되지 않을까? 이를 위해 `rewrite` tactic이 있다.
...
이제 등호를 기준으로 양쪽이 일치하므로 `reflexivity.`를 이용해 증명을 끝낼 수 있다. Qed까지 끝났다면 `rewrite -> H.`를 `rewrite <- H`로 바꿔보고 rewrite가 어떤 식으로 동작하는지 감을 잡아보자.